...
if "Sétimo checkpoint executável — auditoria da fronteira final" not in text:
    block = """
...
d39 = "39. Falha financeira nunca autoriza fallback estático nem refresh de timestamp: `taxas_bacen.json` só é reescrito com dois candidatos atuais `PARSED` e evidência persistida validada."
if "40. `scraper.py` só pode consumir" not in text:
    addition = d39 + """
...
43. `validate_phase4_foundation.py` continua sendo gate de fundação; `validate_phase4_preclosure_gate.py` prova A01–A04 sem autorizar fechamento. A Fase 4 permanece aberta enquanto A05 não tiver política explícita."""
    if d39 not in text:
        raise SystemExit("Decision 39 not found")
    text = text.replace(d39, addition, 1)

old_q = "- revisão final da fronteira entre `taxas.yml`, `main.yml`, `taxas_bacen.json` e `dados_fiscais.json` — fechamento da Fase 4;"
new_q = "- A05: política explícita para consumo (ou invalidação) do last-good financeiro quando a tentativa corrente entra em `PARSER_INCOMPATIBLE` — bloqueia o fechamento formal da Fase 4;"
if old_q in text:
    text = text.replace(old_q, new_q, 1)

old_next = """Continuar a **Fase 4 — Fontes e sensores** com os quatro coletores externos já migrados na branch: RFB, INSS, Selic/SGS 432 e CDI/SGS 12.
...
new_next = """Continuar a **Fase 4 — Fontes e sensores** com a fronteira final de produção corrigida para A01–A04 e protegida pelo pre-closure gate.
...
changelog_marker = "### 2026-09-13 — migração de Selic e CDI na Fase 4\n"
if "### 2026-09-13 — correções A01–A04 da auditoria de fronteira\n" not in text:
    changelog = """### 2026-09-13 — correções A01–A04 da auditoria de fronteira
...
print("README A01-A04 sync: PASS")
