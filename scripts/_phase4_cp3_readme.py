...
anchor = """Segundo checkpoint executável:
...
if anchor not in text:
    raise SystemExit("phase4 cp2 anchor not found")
text = text.replace(anchor, replacement, 1)

decision = "32. Estado operacional de fonte preserva a última observação bem-sucedida, validadores HTTP e falhas correntes sem transformar last-good operacional em autorização jurídica de uso; mudança de versão do parser invalida o atalho condicional e exige refetch.\n"
addition = decision + "33. Para `INSS_TABLE_2026`, o novo pipeline usa exclusivamente a URL canônica registrada; notícia anual pinned e `@@search` não são fallback automático e falha da fonte registrada permanece `SOURCE_UNAVAILABLE`.\n"
if decision not in text:
    raise SystemExit("decision 32 anchor not found")
