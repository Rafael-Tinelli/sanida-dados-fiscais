...
anchor = """- suíte integral chega a **160 testes verdes** no primeiro checkpoint;
- o legado de produção permanece intacto neste checkpoint.
"""
replacement = """- suíte integral chega a **160 testes verdes** no primeiro checkpoint;
- o legado de produção permanece intacto neste checkpoint.
...
if anchor not in text:
    raise SystemExit("phase4 checkpoint anchor not found")
text = text.replace(anchor, replacement, 1)

decision = "31. Na Fase 4, coleta e interpretação são etapas distintas: bytes brutos são preservados e identificados por SHA-256 antes de qualquer parser; indisponibilidade da fonte e incompatibilidade do parser nunca são o mesmo estado.\n"
addition = decision + "32. Estado operacional de fonte preserva a última observação bem-sucedida, validadores HTTP e falhas correntes sem transformar last-good operacional em autorização jurídica de uso; mudança de versão do parser invalida o atalho condicional e exige refetch.\n"
if decision not in text:
    raise SystemExit("decision anchor not found")
text = text.replace(decision, addition, 1)

old_open = "- estado operacional persistente entre execuções (ETag/Last-Modified, última observação e falha corrente) — Fase 4;\n"
if old_open in text:
...
changelog_anchor = "## 22. Changelog do README\n\n"
changelog = """## 22. Changelog do README
...
text = text.replace(changelog_anchor, changelog, 1)
